<template>
  <div class="content-wrapper">
    ...
    <el-form :model="form" :rules="rules" ref="attachmentForm" label-width="180px">
      ...
      <el-form-item label="国家职业资格证书:" class="customUpLoad customUpLoad1" prop="nvq">
        <input
          ref="nvp_file"
          type="file"
          name="nvp"
        ...
        <el-input
          v-model="form.nvq"
          class="uploadInput"
          @click.prevent="$refs.nvp_file.click()"
          placeholder="包括职业资格等级页、照片页、职业工种及成绩页"
        ></el-input>
        <el-button @click.prevent="$refs.nvp_file.click()" type="primary" class="uploadBtn1"
          >上传</el-button
        >
      </el-form-item>
      ...
      <el-form-item label="师带徒情况证明:" class="customUpLoad customUpLoad1" prop="sdtqkzm" >
        <input
          ref="sdtqkzm_file"
          type="file"
          name="performanceCertification"
          style="width:100%;display: none;"
          @change="sdtqkzmChange"
        />
        <el-input
          v-model="form.sdtqkzm"
          class="uploadInput"
          @click.prevent="$refs.sdtqkzm_file.click()"
          placeholder="师带徒情况证明"
        ></el-input>
        <el-button
          @click.prevent="$refs.sdtqkzm_file.click()" class="uploadBtn1"
          type="primary"
          >上传</el-button
        >
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    nvqChange(e) {
      ...
      input.value = "";
      this.form.nvq = file.name

      let id = this.nvqData && this.nvqData.id
    ...
    sdtqkzmChange(e) {
      ...
      input.value = "";
      this.form.sdtqkzm = file.name

      let id = this.sdtqkzmData && this.sdtqkzmData.id
      id && this.$emit('setDelArr', id)
        ...
        attachmentReqDTOS.forEach(e => {
          switch (e.type) {
            ...
            case "HKB": // 户口本
              ...
              this.hkbData = e
              break;
            case "SDTQKZM": // 师带徒情况证明
              this.form.sdtqkzm = e.name;
              this.sdtqkzmData = e
              break;
            case "NVQ": // 国家职业资格证书
              this.form.nvq = e.name;
              this.nvqData = e
              break;
    ...
    getParam(formData) {
      ...
      (this.sdtqkzmData && !this.sdtqkzmData.id) && formData.append("sdtqkzm", this.sdtqkzmData.file);
      (this.nvqData && !this.nvqData.id) && formData.append("nvq", this.nvqData.file);
