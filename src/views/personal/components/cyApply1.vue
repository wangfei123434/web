<template>
  <div class="content-wrapper">
    ...
    <el-form :model="form" :rules="rules" ref="attachmentForm" label-width="180px">
      ...
      <el-form-item label="主要成果:" class="customUpLoad customUpLoad1" prop="cgzm">
        <input
        ...
        ></el-input>
        <el-button
          @click.prevent="$refs.performanceCertification_file.click()" class="uploadBtn1"
          type="primary"
          >上传</el-button
        >
      </el-form-item>
      ...
      <el-form-item label="持股证明:" class="customUpLoad customUpLoad1" prop="gdzm" v-if="title == 'cylj3'">
        <input
...
export default {
  ...
  computed: {
    ...
    rules() {
      if(this.$route.query.title == 'cylj3') {
        return this.rules2
      }
       return this.rules1
    }
    ...
    check() {
      ...
      this.$refs.attachmentForm.validate(valid => {
        if (valid) {
          ...
          console.log("attachmentForm error submit!!", this);
          // Object.keys(this.rules).forEach(k => {
          //   this.$refs.attachmentForm.validate(k)
          // })
          return false;
        }
      });
