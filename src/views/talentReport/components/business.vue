<template>
  ...
  <div class="box">
    <div class="explain">
      ...
      <p>
        3、申报书请用A4纸正反打印，一式7份；其他附件材料请装订成册，报送1份。
      </p>
    </div>
    <hr />
    <!--头部查询 -->
    ...
    >
      ...
      <el-form-item label="籍贯" prop="nativePlace">
        <el-input v-model="form.nativePlace" style="width: 220px"></el-input>
      </el-form-item>
      ...
      <el-form-item label="党派" prop="partisan">
        <el-input v-model="form.partisan"></el-input>
      </el-form-item>
      ...
      <el-form-item label="有无海外学习或工作经历" prop="hasForeignWork">
        <el-radio-group v-model="form.hasForeignWork">
...
20XX年9月-20XX年X月 美国XX大学XX学院XX专业信息技术硕士"
      ...
      <el-form-item label="工作经历" prop="work">
        ...
        <el-input
          ...
          v-model="form.work"
          :rows="5"
          placeholder="（按时间顺序填写至今，包括“时间、国家、单位、职务”，时间不可间断，兼职请注明）如：
0XX年7月-20XX年X月 中国 XXXX有限公司XX部门XX项目组负责人"
        ></el-input>
      </el-form-item>
      <el-form-item label="成果荣誉" prop="honor">
        ...
        <el-input
          ...
          v-model="form.honor"
          :rows="5"
          placeholder="主要荣誉（包括但不限于市级以上创业大赛获奖等荣誉）"
        ></el-input>
      </el-form-item>
      ...
      >
        ...
        <el-input
          ...
          class="uploadInput"
          placeholder="（代表性论著、专利证书、产品证书等）复印件或证明材料，主要荣誉证书复印件"
        ></el-input>
      </el-form-item>
      <!-- 2022-08-25 新增 -->
      <el-form-item
      ...
      >
        <el-button
          @click.prevent="$refs.cbqyzm_file.click()"
          class="uploadBtn1"
          type="primary"
          >上传</el-button
        >
        ...
        />
        <el-input
          v-model="form.cbqyzmAtta"
          class="uploadInput"
          placeholder="（营业执照复印件、验资证明、股权构成证明）"
        ></el-input>
      </el-form-item>
      ...
      >
        <el-button
          @click.prevent="$refs.gsgdzm_file.click()"
          class="uploadBtn1"
          type="primary"
          >上传</el-button
        >
        ...
        />
        <el-input
          v-model="form.gsgdzmAtta"
          class="uploadInput"
          placeholder="工商局出具的创投机构（投资基金）为公司股东的相关证明"
        ></el-input>
      </el-form-item>
      ...
      >
        <el-button
          @click.prevent="$refs.sdtzdyhls_file.click()"
          class="uploadBtn1"
          type="primary"
          >上传</el-button
        >
        <input
          ...
          name="sdtzdyhls"
          style="width: 100%; display: none"
          @change="sdtzdyhlsChange"
          accept="application/msword, image/jpeg, image/png, application/pdf"
        />
        <el-input
          v-model="form.sdtzdyhlsAtta"
          class="uploadInput"
          placeholder="银行的相关流水证明"
        ></el-input>
      </el-form-item>
      <!-- 2022-08-25 新增  end   founderproof    gdzm   sdtzdyhls-->

      <!-- 2022-10-14 新增  grsrzm -->
      <el-form-item
        label="个人收入证明材料:"
        class="customUpLoad customUpLoad1"
        prop="grsrzmAtta"
      >
        <el-button
          @click.prevent="$refs.grsrzm_file.click()"
          class="uploadBtn1"
          type="primary"
          >上传</el-button
        >
        ...
        />
        <el-input
          v-model="form.grsrzmAtta"
          class="uploadInput"
          placeholder="个人相关收入证明"
        ></el-input>
      </el-form-item>
      <!-- 2022-10-14  新增  end -->
      ...
      <div class="foot">
        ...
        <el-button
          @click="submit()"
          class="submitBtn"
          v-if="status == 0 || status == 104 || status == 7"
          :disabled="btnDis"
          >提交</el-button
        >
        <el-button class="submitBtn" v-else disabled>{{
          status == 3
            ? "审核通过"
            : status == 4 || status == 5
            ? "审核不通过"
...
import { citySave } from "@/api/city.js";
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        nativePlace:[{ required: true, message: "请输入内容", trigger: "blur" },],
        ...
        partisan:[{ required: true, message: "请输入内容", trigger: "blur" }],
        post: [{ required: true, message: "请输入内容", trigger: "blur" }],
        currentPostDate: [
          { required: true, message: "请选择", trigger: "blur" },
        ],
        hasForeignWork:[{ required: true, message: "请选择", trigger: "blur" },],
        ...
        cbqyzmAtta: [{ required: true, message: "请上传文件", trigger: "blur" }],
        gsgdzmAtta:[{ required: true, message: "请上传文件", trigger: "blur"}],
        sdtzdyhlsAtta:[{ required: true, message: "请上传文件", trigger: "blur"}],
        grsrzmAtta:[{ required: true, message: "请上传文件", trigger: "blur"}]
      },
      ...
      industryList: [
        ...
        { label: "超级计算", value: "超级计算" },
        { label: "脑科学和类脑科学", value: "脑科学和类脑科学" },
        { label: "深地深海深空", value: "深地深海深空" },
        { label: "其他", value: "其他" },
      ],
      //       editorOption1: {
      //         placeholder: `（按时间顺序，从本科填起，包括“时间、国家、院校、专业、学位”）
  ...
  methods: {
            ...
            attachment.forEach((e) => {
              ...
              switch (type) {
                ...
                case "dwtjh":
                  ...
                  break;
                case "founderproof":
                  that.$set(that.form, "cbqyzmAtta", e.name);
                  that.cbqyzmAttaData = e;
                  break;
                case "gdzm":
                  that.$set(that.form, "gsgdzmAtta", e.name);
                  that.gsgdzmAttaData = e;
                  break;
                case "sdtzdyhls":
                  that.$set(that.form, "sdtzdyhlsAtta", e.name);
                  that.sdtzdyhlsAttaData = e;
                  break;
                case "grsrzm":
                  that.$set(that.form, "grsrzmAtta", e.name);
                  that.grsrzmAttaData = e;
                  break;
              }
            });
          }
    ...
    cbqyzmChange(e) {
      ...
      input.value = "";
      // this.form.cgzmAtta = file.name
      this.$set(this.form, "cbqyzmAtta", file.name);

      let id = this.cbqyzmAttaData?.id;
      id && this.delArr.push(id);
      ...
      this.$refs.form.clearValidate("cbqyzmAtta");
    },
    ...
    gsgdzmChange(e) {
      ...
      input.value = "";
      // this.form.cgzmAtta = file.name
      this.$set(this.form, "gsgdzmAtta", file.name);

      let id = this.gsgdzmAttaData?.id;
      id && this.delArr.push(id);
      ...
      this.$refs.form.clearValidate("gsgdzmAtta");
    },
    ...
    sdtzdyhlsChange(e) {
      ...
      input.value = "";
      // this.form.cgzmAtta = file.name
      this.$set(this.form, "sdtzdyhlsAtta", file.name);

      let id = this.sdtzdyhlsAttaData?.id;
      id && this.delArr.push(id);

      this.sdtzdyhlsAttaData = {
        file,
        name: file.name,
        time: Date.now(),
      };

      this.$refs.form.clearValidate("sdtzdyhlsAtta");
    },
    ...
    grsrzmChange(e) {
      ...
      input.value = "";
      // this.form.cgzmAtta = file.name
      this.$set(this.form, "grsrzmAtta", file.name);

      let id = this.grsrzmAttaData?.id;
      id && this.delArr.push(id);
      ...
      this.$refs.form.clearValidate("grsrzmAtta");
    },
    ...
    getParam(operationType) {
      ...
      this.cbqyzmAttaData?.file &&
        formData.append("founderproof", this.cbqyzmAttaData.file);
      this.gsgdzmAttaData?.file &&
        formData.append("gdzm", this.gsgdzmAttaData.file);
      this.sdtzdyhlsAttaData?.file &&
        formData.append("sdtzdyhls", this.sdtzdyhlsAttaData.file)
      this.grsrzmAttaData?.file &&
        formData.append("grsrzm", this.grsrzmAttaData.file)
      return formData;
    },
    save(operationType) {
      let param = this.getParam(operationType);
      citySave(param).then((res) => {
        console.log(res);
        if (res?.code == 200) {
          let t;
          if (operationType == 0) {
    ...
    checkAttaType() {
      ...
      n = this.gsgdzmAttaData?.name.split(".");
      if (n.length < 2) return false;
      f = t.includes(n[n.length - 1]);
      if (!f) return false;

      n = this.sdtzdyhlsAttaData?.name.split(".");
      if (n.length < 2) return false;
      f = t.includes(n[n.length - 1]);
      if (!f) return false;

      n = this.grsrzmAttaData?.name.split(".");
      if (n.length < 2) return false;
      f = t.includes(n[n.length - 1]);
      if (!f) return false;
